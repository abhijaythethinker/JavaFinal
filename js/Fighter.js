...
class Fighter extends Sprite {
  ...
  isHitting(enemyFighter) {
    return (
      ...
      this.attackBox.position.x <=
        enemyFighter.position.x + enemyFighter.width &&
  ...
  update() {
    ...
    if (this.position.y + this.height + this.velocity.y >= canvas.height - 95) {
      // Sprite reach the bottom of the canvas.
      this.velocity.y = 0;
    ...
    switch (sprite) {
      ...
      case "run":
        if (!this.isAttacking && !this.isTakingHit) {
          // Can only run if is not attacking or taking a hit
          //console.log("correr")
      ...
      case "fall":
        if (this.image !== this.sprites.attack1.image) {
          // Show attack anim while falling.
          //console.log("caer")
...
export const player = new Fighter({
  ...
  imageSrc: "/assets/img/BigDev/Idle.png",
  scale: 2.5,
  maxFrames: 1,
  holdFrames: 4,
  offsetFrame: { x: 0, y: 30 },
  sprites: {
